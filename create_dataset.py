...
if os.path.exists(phon_path) and not args.RECOMPUTE_PHON:
    ...
    texts = audio_data[:, 1]
    batch_size = 50  # batch phonemization to avoid memory issues.
    phonemes = []
    print(len(audio_data))
    for i in tqdm.tqdm(range(0, 1300, batch_size)):
        batch = texts[i: i + batch_size]
        print('size',batch.size)
        batch = text_proc.phonemizer(batch, njobs=args.NJOBS)
        phonemes.extend(batch)

    audio_data = np.concatenate([np.array(audio_data[0:1299]), np.expand_dims(phonemes, axis=1)], axis=1)
    if args.CACHE_PHON:
...
for i in tqdm.tqdm(range(len(audio_data))):
    ...
    wav_path = os.path.join('libritts/new1/'+filename + '.wav')
