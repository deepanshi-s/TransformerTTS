...
class Dataset:
    ...
    def __init__(self,
        ...
        padded_shapes = ([-1, mel_channels], [-1], [-1], [-1, x_vec_shape])
...
def load_files(metafile,
    ...
    with open(metafile, 'r', encoding='utf-8') as f:
        for l in f.readlines():
            l_split = l.split('|') #change here
            mel_file = os.path.join(str(meldir), l_split[0] + '.npy')
            #add xvec file here
            xvec_file = os.path.join('libritts/new1/xvectors/', l_split[0] + '.txt')
            text = l_split[1].strip().lower()
            phonemes = l_split[2].strip()
            #add xvectors to samples
            samples.append((phonemes, text, mel_file, xvec_file))
...
class DataPrepper:
    ...
    def __call__(self, sample):
        phonemes, text, mel_path, xvec = sample
        mel = np.load(mel_path)
        x_vec = np.loadtxt(xvec)
        return self._run(phonemes, text, mel, x_vec)
    
    def _run(self, phonemes, text, mel, x_vec):
        ...
        stop_probs[-1] = 2
        x_vec = x_vec.reshape((512,1))
        return norm_mel, encoded_phonemes, stop_probs, x_vec
